...
<body>
    ...
    <div class="displaycontainer">
    	<div class="displaybox">
			...
			$sql="SELECT games.game , games.slot1, games.slot2 , games.slot3 , employee.name
            FROM sports
            INNER JOIN games ON sports.sport=games.game INNER JOIN employee ON employee.sport=sports.sport
			...
			if(mysqli_num_rows($result)>0)
            {
			    ...
			    while($row=mysqli_fetch_assoc($result)){
                    echo "<div style='display:inline-block; justify-content:space-around;'><h3 style='color:rgb(208, 255, 38); padding: 8px 25px;'>".$row['game']."</h3><a id='trainer' href='contactus.html'>".$row['name']."</a><h5 style='padding: 5px 25px;'>".$row['slot1']."<br>".$row['slot2']."<br>".$row['slot3']."</h5></div>";
			    }
			}
